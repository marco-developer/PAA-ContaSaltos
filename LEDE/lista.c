...
// OBSERVAÇÃO: LINHAS COM O COMENTÁRIOS COM //D SÃO ADIÇÕES
// PARA ANALISAR A QUANTIDADE DE SALTOS!
...
int inserirInicioVazio(lLista l, void *elemento, int * saltos) {
  ...
  *saltos = *saltos + 1; //D
  printf("[lista.c] Numero de saltos: %d\n", *saltos); //D
  return TRUE;
}

// insere no inicio com a lista vazia
int inserirInicioNaoVazio(lLista l, void *elemento, int * saltos) {
...
int inserirInicio(lLista l, void *elemento, int * saltos) {
  // verifica se a lista ja não esta vazia  
  if (l == NULL || !l || l->tamanho_total_lista <= 0) {
    return FALSE;
  }
  ...
  if (l->posicao_primeiro_elemento == EMPTY) {
    return inserirInicioVazio(l, elemento, saltos);
  }
  return inserirInicioNaoVazio(l, elemento, saltos);
}

// insere no fim com a lista vazia
int inserirFimVazio(lLista l, void *elemento, int * saltos) {
  return inserirInicioVazio(l, elemento, saltos);
  // Se você parar para pensar, a lógica é a mesma
}

// insere no fim quando a lista tem elementos
int inserirFimNaoVazio(lLista l, void *elemento, int * saltos) {
...
// element -> ponteiro do elemento a ser inserido
// retorna TRUE caso de certo e FALSE caso de errado
int inserirFim(lLista l, void *elemento, int * saltos) {
  if (l->posicao_ultimo_elemento == EMPTY) {
    return inserirFimVazio(l, elemento, saltos);
  }
  return inserirFimNaoVazio(l, elemento, saltos);
}
...
// retorna TRUE caso de certo e FALSE caso de errado
int inserirNaPosicao(lLista l, void *elemento, int posicao, int * saltos) {
  int pos = procurarVazio(l);
...
// posicao -> posicao a inserir o elemento na lista
// retorna TRUE caso de certo e FALSE caso de errado
int inserirPos(lLista l, void *elemento, int posicao, int * saltos) {
  // verifica se a lista ja não esta vazia
  printf("\n[lista.c] Tamanho atual da lista: %d\n", l->tamanho_atual); //D
  if (l == NULL || !l || l->tamanho_total_lista <= 0) {
    return FALSE;
  }
  if (posicao < 0 || posicao >= l->tamanho_atual) {
    ...
    printf("[lista.c] Inserindo no Inicio!\n"); //D
    return inserirInicio(l, elemento, saltos);
  } else if (posicao == l->tamanho_atual - 1) {
    printf("[lista.c] Inserindo no Fim!\n"); //D
    return inserirFim(l, elemento, saltos);
  }
  printf("[lista.c] Inserindo Na Posicao!\n"); //D
  return inserirNaPosicao(l, elemento, posicao, saltos);
}

// removerInicio retorna Null caso a lista esteja vazia ou retorna o elemento removido
void *removerInicio(lLista l, int * saltos) {
  return removerPos(l, l->posicao_primeiro_elemento, saltos);
}
...
void removerPrimeiroElem(lLista l, int pos, int * saltos) {
...
void removerUltimoElem(lLista l, int pos, int * saltos) {
  if (l->posicoes_ocupadas[pos] == TRUE) {
    l->elementos[pos]->posicao_proximo = l->posicao_primeiro_elemento;
  }
...
void *removerPos(lLista l, int posicao, int * saltos) {
  // verifica se a lista ja não esta vazia
  if (l == NULL || !l || l->tamanho_total_lista <= 0) {
    return NULL;
  }
  ...
  if (l->posicao_primeiro_elemento == posicao) {
    ...
    removerPrimeiroElem(l, pos_proximo, saltos);
    *saltos = *saltos + 1; //D
    printf("Numero de saltos: %d\n", *saltos); //D
    return dado;
  }
  ...
  if (l->posicao_ultimo_elemento == posicao) {
    ...
    removerUltimoElem(l, pos_anterior, saltos);
    *saltos = *saltos + 1; //D
    printf("Numero de saltos: %d\n", *saltos); //D
    return dado;
  }
...
void *removerFim(lLista l, int * saltos) {
  return removerPos(l, l->posicao_ultimo_elemento, saltos);
}
